...
void computePhi(const Eigen::Matrix<float,3,1> &f_i,const Eigen::Matrix<float,3,3> &R_body_to_nav_next, Eigen::Matrix<float,9,9> &Phi)
{
	...
	float lambda_g = -1.0/1000;
	float lambda_a = -1.0/1000;
	F.block<3,3>(0,3) = -1*R_body_to_nav_next;
	F.block<3,3>(3,3) = Eigen::Matrix<float,3,3>::Identity()*lambda_g; // Fg
	F.block<3,3>(6,6) = Eigen::Matrix<float,3,3>::Identity()*lambda_a; // Fa
...
void computeQdk(const Eigen::Matrix<float,3,3> &R_body_to_nav_next, Eigen::Matrix<float,9,9> &Qdk)
{
	...
	Eigen::Matrix<float,9,12> G = Eigen::Matrix<float,9,12>::Zero();
	G.block<3,3>(0,0) = -1*R_body_to_nav_next;
	G.block<3,3>(0,6) = -1*R_body_to_nav_next;
	G.block<3,3>(3,3) = Eigen::Matrix<float,3,3>::Identity();
	G.block<3,3>(6,9) = Eigen::Matrix<float,3,3>::Identity();
...
void stationaryMeasurementUpdate(const Eigen::Matrix<float,3,3> & R_body_to_nav)
{
	...
	Eigen::Matrix<float,3,9> Ha = Eigen::Matrix<float,3,9>::Zero();
...
void EKF(const Eigen::MatrixXf & H, const Eigen::MatrixXf & R, const Eigen::MatrixXf & z, const bool update_bias)
{
	...
	Eigen::Matrix<float,9,1> dx = K*z;

	// 	rotation update 
	Eigen::Matrix<float,3,1> ro = dx(Eigen::seq(0,2));

	Eigen::Matrix<float,3,3> P(3,3);
	to_skew(ro,P);
	// predicted orientation
	Eigen::Quaternion<float> b_q = Eigen::Quaternion<float>(state[0],state[1],state[2],state[3]);
	...
	// update covariance
	cov = (Eigen::Matrix<float,9,9>::Identity()-K*H)*(cov);
	// symmetrify again
	cov = (cov + cov.transpose())/2.0;
...
void imu_callback(const sensor_msgs::Imu::ConstPtr& msg)
{
	...
	// current orientation
	Eigen::Matrix<float,4,1> b = state(Eigen::seq(0,3));
	Eigen::Quaternion<float> b_prev = Eigen::Quaternion<float>(b(0),b(1),b(2),b(3)); // w, x, y, z

	// current accel bias
	Eigen::Matrix<float,3,1> x_a = state(Eigen::seq(4,6));

	// current gyro bias
	Eigen::Matrix<float,3,1> x_g = state(Eigen::seq(6,8));
